...
class ApicodeVN {
    constructor() {
        this.container = null;
        this.host = 'https://cdn.jsdelivr.net/gh/ngockush/toaster_apicodevn@main';
    }
    status_detect(status) {
        if (status == 'warning') {
            return {
                icon: this.host + "/apicodevn_alert/assets/image/595067.png",
                alertClass: "apicodevn-warning"
            }
        } else if (status == 'danger') {
            return {
                icon: this.host + "/apicodevn_alert/assets/image/2737906.png",
                alertClass: "apicodevn-danger"
            }
        } else if (status == 'success') {
            return {
                icon: this.host + "/apicodevn_alert/assets/image/7518748.png",
                alertClass: "apicodevn-success"
            }
        } else if (status == 'info') {
            return {
                icon: this.host + "/apicodevn_alert/assets/image/3674690.png",
                alertClass: "apicodevn-info"
            }
        } else if (status == 'loading') {
            return {
                icon: this.host + "/apicodevn_alert/assets/image/loading.svg",
                alertClass: "apicodevn-secondary"
            }
        } else {
            return {
                icon: this.host + "/apicodevn_alert/assets/image/1827370.png",
                alertClass: ""
            }
        }
    }
    ...
    creater_body(args) {
            ...
            this.container.html(`<div class="alert-container ${alertClass}"><div class="alert-body">
                                    ...
                                    <button type="button" class="btn-alert-close"><img src="${this.host}/apicodevn_alert/assets/image/9068678.png" alt="close alert"></button>
                                </div></div>`);
        this.container.hide();
    }
    toast(args) {
        ...
        var promise = new Promise(function (resolve) {
            jQuery('.btn-alert-close').click(function () {
                toaster.remove_alert();
                resolve();
            });
        });
