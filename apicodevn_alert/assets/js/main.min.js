document.addEventListener("DOMContentLoaded", function(e) {
    ...
    link.rel = 'stylesheet';
    link.href = '/apicodevn_alert/assets/css/style.min.css';
    var head = document.head || document.getElementsByTagName('head')[0];
    head.appendChild(link);
    if (!window.jQuery) {
        var jquery_script = document.createElement('script');
        jquery_script.src = '/apicodevn_alert/assets/js/jquery.js';
...
class ApicodeVN {
    ...
    status_detect(status) {
        if (status == 'warning') {
            return {
                icon: "/apicodevn_alert/assets/image/595067.png",
                alertClass: "apicodevn-warning"
            }
        } else if (status == 'danger') {
            return {
                icon: "/apicodevn_alert/assets/image/2737906.png",
                alertClass: "apicodevn-danger"
            }
        } else if (status == 'success') {
            return {
                icon: "/apicodevn_alert/assets/image/7518748.png",
                alertClass: "apicodevn-success"
            }
        } else if (status == 'info') {
            return {
                icon: "/apicodevn_alert/assets/image/3674690.png",
                alertClass: "apicodevn-info"
            }
        } else if (status == 'loading') {
            return {
                icon: "/apicodevn_alert/assets/image/loading.svg",
                alertClass: "apicodevn-secondary"
            }
        } else {
            return {
                icon: "/apicodevn_alert/assets/image/1827370.png",
                alertClass: ""
            }
        }
    }
    ...
    creater_body(args) {
            ...
            this.container.html(`<div class="alert-container ${alertClass}"><div class="alert-body">
                                    ...
                                    <button type="button" class="btn-alert-close"><img src="/apicodevn_alert/assets/image/9068678.png" alt="close alert"></button>
                                </div></div>`);
        this.container.hide();
    }
    toast(args) {
